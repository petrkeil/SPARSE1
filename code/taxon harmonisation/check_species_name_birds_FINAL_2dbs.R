...
# Description: The script contains a function that takes a species list as input
# and returns a dataframe with two columns: Species Name and Observation. The 
# run will return the result of the check for each species in the list, doing it
# first with ITIS database (Integrated Taxonomic Information System) and then with
# IUCN database (IUCN Red List of Threatened Species) case the species is not found
# in ITIS. If the species has a match in any of the databases, it will return the
# same Species Name and 'Ok ITIS' or 'Ok IUCN' as Observation. If the species has
# a match with any error of spelling, it will return the matched correct name 
# in Species Name and 'Checked ITIS' or 'Checked IUCN' as Observation. If the species
# is not found in any of the databases, it will return 'NOT FOUND in ITIS or IUCN' as Observation.
...
CZ_birds <- read_csv('speciesAccess_23-01.csv')
head(CZ_birds)
